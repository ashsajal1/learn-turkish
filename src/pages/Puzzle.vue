  ...
  <div class="puzzle-container p-4 md:p-6 max-w-4xl mx-auto">
    ...
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-2">
        Turkish-Bengali Word Match
      </h1>
      <p class="text-gray-600 dark:text-gray-400">
        Match the Turkish words with their Bengali translations
      </p>
      ...
      <div class="flex justify-center items-center gap-4 mt-4">
        ...
        <div class="flex items-center">
          <span
            class="w-4 h-4 bg-blue-100 dark:bg-blue-900 rounded-full border border-blue-500 mr-2"
          ></span>
    ...
    <div class="grid grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        <h2 class="text-xl font-semibold mb-4 text-center text-blue-600 dark:text-blue-400">
          Turkish Words
          <span class="text-base text-gray-500 dark:text-gray-300">/ Türkçe Kelimeler</span>
        </h2>
        <div class="space-y-3">
          <div
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        <h2
          class="text-xl font-semibold mb-4 text-center text-green-600 dark:text-green-400"
        >
    ...
    <div class="text-center">
      <div v-if="!gameComplete" class="text-lg font-medium mb-4">
        Matches: <span class="font-bold">{{ matches.length / 2 }}</span> /
        {{ turkishWords.length }}
      </div>
...
const getWordClasses = (type: "turkish" | "bengali", index: number) => {
  const isSelected = selectedWords.value.some(
    (sw) => sw.type === type && sw.index === index
  );
...
const checkMatch = () => {
  ...
  if (
    firstWordObject.index === secondWordObject.index &&
...
const selectWord = (type: "turkish" | "bengali", index: number) => {
  ...
  if (
    selectedWords.value.some((sw) => sw.type === type && sw.index === index)
  ) {
...
const startNewGame = () => {
  ...
  shuffledBengaliWords.value = [...bengaliWords.value].sort(
    () => 0.5 - Math.random()
  );
};
...
onMounted(() => {
  ...
  toast.add({
    ...
    summary: "How to Play",
    detail:
      "Match the Turkish words with their Bengali translations by clicking on pairs.",
    life: 5000,
  });
});
</script>
